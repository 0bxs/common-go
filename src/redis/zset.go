...
var zmScoreAndRankScript = redis.NewScript(`
local result = {}
...
type ZScoreAndRankElement[T types.Number | string] struct {
	Member T
	Score  float64
	Rank   int64
}

func ZMScoreAndRank(key string, members []string) vec.Vec[ZScoreAndRankElement[string]] {
	raw := catch.Try1(zmScoreAndRankScript.Run(context.Background(), client, []string{key}, members).Text())
	v := new(vec.Vec[ZScoreAndRankElement[string]])
	if raw == "{}" {
		return vec.OfEmpty[ZScoreAndRankElement[string]]()
	}
	catch.Try(sonic.Unmarshal([]byte(raw), v))
	return *v
}
...
func zRangeByScoreObj[T any](key string, start, end, offset, limit int64, rev bool) vec.Vec[ZElement[T]] {
	args := make([]any, 0, 9)
	start0 := trans.I642Str(start)
	end0 := trans.I642Str(end)
	if start < 0 {
		start0 = "-inf"
	}
	if end < 0 {
	...
	args = append(args, "zrange", key, start0, end0)
	if rev {
		args = append(args, "rev")
	}
...
func ZRangeRevByScoreObj[T any](key string, start, end int64) vec.Vec[ZElement[T]] {
	return zRangeByScoreObj[T](key, start, end, 0, 0, true)
}

func ZRangeByScoreObj[T any](key string, start, end int64) vec.Vec[ZElement[T]] {
	return zRangeByScoreObj[T](key, start, end, 0, 0, false)
}

func ZRangeByScoreObj1[T any](key string, start, end, offset, limit int64) vec.Vec[ZElement[T]] {
	return zRangeByScoreObj[T](key, start, end, offset, limit, false)
}
...
func zRangeByScore(key string, start, end, offset, limit int64, rev bool) vec.Vec[ZElement[string]] {
	args := make([]any, 0, 9)
	start0 := trans.I642Str(start)
	end0 := trans.I642Str(end)
	if start < 0 {
...
func ZRangeByScore(key string, start, end int64) vec.Vec[ZElement[string]] {
	return zRangeByScore(key, start, end, 0, 0, false)
}

func ZRangeByScore1(key string, start, end, offset, limit int64) vec.Vec[ZElement[string]] {
	return zRangeByScore(key, start, end, offset, limit, false)
}

func ZRangeRevByScore(key string, start, end int64) vec.Vec[ZElement[string]] {
	return zRangeByScore(key, start, end, 0, 0, true)
}
