...
func SetNxAndPx[T any](key string, value T, expire int64) bool {
	res, err := client.Do(context.Background(), "set", key, value, "nx", "px", expire).Text()
	try(err)
	if res == "OK" {
		return true
	}
...
func IncrBy1(key string, increment int64) int64 {
	return catch.Try1(client.IncrBy(context.Background(), key, increment).Result())
}

func MGet(keys vec.Vec[string]) []any {
	return catch.Try1(client.MGet(context.Background(), keys...).Result())
}
...
func GetI64Del(key string) option.Opt[int64] {
	result, err := client.GetDel(context.Background(), key).Int64()
	return option.OptOf[int64](result, try(err))
}

func GetU32Del(key string) option.Opt[uint32] {
	result, err := client.GetDel(context.Background(), key).Int()
	return option.OptOf[uint32](uint32(result), try(err))
}

func GetBytesDel(key string) option.Opt[[]byte] {
	result, err := client.GetDel(context.Background(), key).Bytes()
	return option.OptOf[[]byte](result, try(err))
}
