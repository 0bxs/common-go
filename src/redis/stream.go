...
func XReadGroups[T any](key, group, consumer string, count int64) option.Opt[vec.Vec[XMessage[T]]] {
	...
	if err != nil || len(xStreams) <= 0 {
		...
		log.Zap.Errorf("获取消息失败: key:%s,group:%s,err:%v", key, group, err)
		return option.OptOfEmpty[vec.Vec[XMessage[T]]]()
	}
	stream := xStreams[0]
...
func XRange[T any](stream, start, stop string) option.Opt[vec.Vec[XMessage[T]]] {
	messages, err := client.XRange(context.Background(), stream, start, stop).Result()
	if err != nil {
		log.Zap.Errorf("获取消息失败: key:%s,err:%v", stream, err)
		return option.OptOfEmpty[vec.Vec[XMessage[T]]]()
	}
	if len(messages) <= 0 {
		return option.OptOfEmpty[vec.Vec[XMessage[T]]]()
