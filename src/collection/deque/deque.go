...
type DequePin[E any] struct {
	data []E
	head int
	tail int
}

// New Create a Deque[E] with a specified capacity
func New[E any](cap_ int) Deque[E] {
	return Deque[E]{&DequePin[E]{make([]E, 0, cap_), 0, 0}}
}
...
// PushFront CrateChatRoom an element to the front
...
// PushBack CrateChatRoom an element to the back
...
func (d Deque[E]) PopFront() option.Opt[E] {
	if d.Empty() {
		return option.OptOf(*new(E), false)
	}
	value := d.data[d.head]
	if d.clearLen() {
		d.data[d.head] = *new(E)
	...
	return option.OptOf(value, true)
}

// PopBack Pop elements from the back
func (d Deque[E]) PopBack() option.Opt[E] {
	if d.Empty() {
		return option.OptOf(*new(E), false)
	}
	value := d.data[d.tail]
...
func (d Deque[E]) Front() option.Opt[E] {
	if d.Empty() {
		return option.OptOf(*new(E), false)
	}
	return option.OptOf(d.data[d.head], true)
}

// Back View the back element
func (d Deque[E]) Back() option.Opt[E] {
	if d.Empty() {
		return option.OptOf(*new(E), false)
	}
	return option.OptOf(d.data[d.tail], true)
}
...
func (d Deque[E]) ToVec() vec.Vec[E] {
	if d.Empty() {
		return vec.Vec[E]{}
	}
	data := vec.New[E](d.Len())
	if d.head > d.tail {
