...
func (bm *BytesBitMap) ToSet() set.Set[int16] {
	length := len(bm.value) * 8
	res := set.New[int16](length / 2)
	for i := range length {
		if bm.get(i) {
			res.Insert(int16(i))
		}
...
func (bm *BytesBitMap) ToVec() vec.Vec[int16] {
	length := len(bm.value) * 8
	res := vec.New[int16](length / 2)
	for i := range length {
		if bm.get(i) {
			res.Append(int16(i))
		}
...
func (bm *BytesBitMap) ToStr() string {
	return string(bm.value)
}
